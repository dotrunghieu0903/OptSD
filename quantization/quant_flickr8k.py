...
from quantization.preprocessing_flickr8k import process_flickr8k
...
def main(args=None):
    ...
    if args is None:
        parser = argparse.ArgumentParser(description="Quantization with Flickr8k dataset")
        parser.add_argument("--precision", type=str, default=None, 
                            help="Precision to use (default is auto-detected)")
        parser.add_argument("--num_images", type=int, default=500,
                            help="Number of Flickr8k images to process")
    ...
    captions, image_dimensions = process_flickr8k()
    if not captions:
        print("Error: No captions loaded from Flickr8k dataset")
        return
    ...
    # 2. Generate images for Flickr8k captions
    print(f"\n=== Generating Images for Flickr8k Captions ===")
    
    # Limit the number of images to generate based on the command-line argument
    num_images_to_generate = min(args.num_images, len(captions))
    ...
    # 3. Calculate Image Quality Metrics (if not skipped)
    ...
    with open(summary_file, "w", encoding="utf-8") as f:
        ...
        if successful_generations:
            ...
            if all_vram_data:
                f.write(f"- Average VRAM usage: {overall_avg_vram:.2f} GB\n")
