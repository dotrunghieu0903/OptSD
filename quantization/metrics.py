...
from dataset.coco import preprocessing_coco
...
def main(args=None):
    ...
    if args is None:
        ...
        parser.add_argument("--caption_path", type=str, default=None,
                            help="Path to the COCO captions file")
    ...
    val2017_dir = args.images_path

    # Process COCO dataset
    print("\n=== Loading COCO Captions ===")
    image_filename_to_caption, image_dimensions, _ = preprocessing_coco(annotations_dir)
    ...
    if not args.skip_metrics:
        ...
        try:
            ...
            for filename in tqdm(selected_filenames, desc="Resizing original images for LPIPS"):
                original_path = os.path.join(val2017_dir, filename)
        ...
        try:
            ...
            # Use the metrics_subset parameter to limit the number of images for metrics
            subset_size = min(args.metrics_subset, len(generated_filenames))
            psnr_score = calculate_psnr_resized(val2017_dir, resized_output_dir, generated_filenames[:subset_size])
