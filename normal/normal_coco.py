...
# Make sure preprocessing_coco can be imported
...
def load_model(config, model_name=None):
    ...
    try:
        # If no model_name provided, use the first model in the config
        if not model_name:
        ...
        # Create a pipeline using the model directly
...
def main(args=None):
    ...
    if args is None:
        parser = argparse.ArgumentParser(description="Normal (non-optimized) image generation with COCO dataset")
    ...
    # 1. Load the model without optimization
    print("\n=== Loading Non-optimized Model ===")
    try:
        pipeline = load_model(config, args.model_name)
        model_size = get_model_size(pipeline.transformer)
        print(f"Non-optimized model size: {model_size:.2f} MB")
    except Exception as e:
        print(f"Error loading model: {e}")
        print("Trying again with default parameters...")
        try:
            # Fallback to default model
            if "Flux.1-dev" in config["models"]:
                model_path = config["models"]["Flux.1-dev"]["path"]
            else:
                model_path = "black-forest-labs/FLUX.1-dev"
            pipeline = FluxPipeline.from_pretrained(model_path, torch_dtype=torch.bfloat16).to("cuda")
            pipeline.enable_model_cpu_offload()
            model_size = get_model_size(pipeline.transformer)
            print(f"Loaded fallback model with size: {model_size:.2f} MB")
    ...
    metadata_file = os.path.join(output_dir, "normal_metadata.json")
    ...
    # Save summary report
    summary_file = os.path.join(output_dir, "normal_summary.txt")
    with open(summary_file, "w", encoding="utf-8") as f:
        f.write("=== Non-optimized Model Summary ===\n\n")
